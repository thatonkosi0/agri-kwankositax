...
// Bundling many files into a zip can be slow; allow up to 60s on serverless.
      ...
      for (const transaction of chunk) {
        ...
        for (const file of transactionFiles) {
          ...
          if (await fileExists(fileKey)) {
            ...
            const fileData = await getStorage().read(fileKey)
            const fileExtension = path.extname(file.filename) || path.extname(fileKey)
