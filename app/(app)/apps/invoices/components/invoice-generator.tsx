...
// Resolve the logo to an inline base64 data URL for PDF rendering without mutating
// form state. Data URLs are already inline; paths/blob URLs are fetched and inlined.
async function withResolvedLogo(formData: InvoiceFormData): Promise<InvoiceFormData> {
  if (!formData.businessLogo || formData.businessLogo.startsWith("data:")) {
    return formData
  }
  const businessLogo = await fetchAsBase64(formData.businessLogo)
...
export function InvoiceGenerator({
  ...
  const handleGeneratePDF = async (e: React.FormEvent) => {
    ...
    try {
      const resolvedFormData = await withResolvedLogo(formData)

      const pdfBuffer = await generateInvoicePDF(resolvedFormData)
  ...
  const handleSaveAsTransaction = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const result = await saveInvoiceAsTransactionAction(resolvedFormData)
