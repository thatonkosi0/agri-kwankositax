...
import { storageKey, userScope } from "@/lib/files"
...
export async function restoreBackupAction(
  ...
  try {
    ...
    if (REMOVE_EXISTING_DATA) {
      ...
      await storage.removePrefix(userScope(user))
    }
    ...
    try {
      ...
      for (const file of files) {
        const filePathWithoutPrefix = file.path.replace(/^.*\/uploads\//, "").replace(/\\/g, "/")
        const zipFilePath = path.posix.join("data/uploads", filePathWithoutPrefix)
        ...
        try {
          // storageKey guards against path traversal in the restored path.
          await storage.write(storageKey(user, filePathWithoutPrefix), fileContents, file.mimetype)
          restoredFilesCount++
        } catch (error) {
