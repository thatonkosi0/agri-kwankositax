...
export async function saveFileAsTransactionAction(
  ...
  try {
    ...
    const originalFileName = file.path.split("/").pop() || file.filename
    const newRelativeFilePath = getTransactionFileUploadPath(file.id, originalFileName, transaction)

    // Move file to new key (storage has no native move: copy then delete)
    const storage = getStorage()
    const contents = await storage.read(storageKey(user, file.path))
    await storage.write(storageKey(user, newRelativeFilePath), contents, file.mimetype)
    await storage.remove(storageKey(user, file.path))
...
export async function splitFileIntoItemsAction(
  ...
  try {
    ...
    const storage = getStorage()
    const fileContent = await storage.read(storageKey(user, originalFile.path))
    ...
    for (const item of items) {
      ...
      // Copy the original file content to the new key
      await storage.write(storageKey(user, relativeFilePath), fileContent, originalFile.mimetype)
