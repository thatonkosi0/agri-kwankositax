          ...
          <FormAvatar
            ...
            onChange={(e) => {
              ...
              if (file.size > MAX_LOGO_SIZE_BYTES) {
                alert("Logo is too large. Please choose an image under 1 MB.")
                e.target.value = ""
                return
              }

              // Read as a base64 data URL so the logo persists with the template
              // (blob: URLs are session-scoped and break after a page reload).
              const reader = new FileReader()
              reader.onloadend = () => {
                dispatch({ type: "UPDATE_FIELD", field: "businessLogo", value: reader.result as string })
              }
              reader.onerror = () => {
                alert("Failed to read the logo image. Please try another file.")
              }
