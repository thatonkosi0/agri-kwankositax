...
const envSchema = z.object({
  ...
  RESEND_FROM_EMAIL: z.string().default("Agri-KwankosiTax <user@localhost>"),
  RESEND_AUDIENCE_ID: z.string().default(""),
  STRIPE_SECRET_KEY: z.string().default(""),
  STRIPE_WEBHOOK_SECRET: z.string().default(""),
  // Storage: "local" writes to the filesystem (self-hosted/Docker); "supabase"
  // uses Supabase Storage (serverless hosts like Vercel where the FS is ephemeral).
  STORAGE_BACKEND: z.enum(["local", "supabase"]).default("local"),
  SUPABASE_URL: z.string().default(""),
  // Supabase's newer dashboard calls this the "secret key" (SUPABASE_SECRET_KEY);
  // older setups use SUPABASE_SERVICE_ROLE_KEY. Accept either.
